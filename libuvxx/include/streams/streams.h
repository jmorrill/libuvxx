...
namespace uvxx { namespace streams
{
    ...
    template<typename CharType>
        ...
        private:
            ...
            uvxx::streams::streambuf<CharType> m_buffer;
        };
    ...
    public:
        ...
        uvxx::streams::streambuf<CharType> streambuf() const
        {
...
#pragma endregion
    ...
    public:
        ...
        pplx::task<size_t> read_to_delim(streams::streambuf<CharType> target, int_type delim) const
        {
            ...
            int_type req_async = uvxx::streams::char_traits<CharType>::requires_async();
        ...
        pplx::task<size_t> read_line(streams::streambuf<CharType> target) const
        {
            ...
            // Capture 'buffer' rather than 'helper' here due to VC++ 2010 limitations.
            uvxx::streams::streambuf<CharType> buffer = helper()->m_buffer;

            typename std::char_traits<CharType>::int_type req_async = uvxx::streams::char_traits<CharType>::requires_async();
    ...
    private:
        ...
        public:
            _dev10_ice_workaround(streams::streambuf<CharType> buffer,
                                  uvxx::streams::streambuf<CharType> target,
        ...
        private:
            size_t _buf_size;
            uvxx::streams::streambuf<CharType> _buffer;
            uvxx::streams::streambuf<CharType> _target;
...
template<typename CharType>
pplx::task<void> uvxx::streams::_type_parser_base<CharType>::_skip_whitespace(streams::streambuf<CharType> buffer)
{
    int_type req_async = uvxx::streams::char_traits<CharType>::requires_async();
...
template<typename CharType>
template<typename _ParseState, typename _ReturnType>
pplx::task<_ReturnType> uvxx::streams::_type_parser_base<CharType>::_parse_input(
    uvxx::streams::streambuf<CharType> buffer,
    std::function<bool(std::shared_ptr<_ParseState>, int_type)> accept_character, 
    std::function<pplx::task<_ReturnType>(std::shared_ptr<_ParseState>)> extract)
{
    ...
    auto update = [=] (pplx::task<int_type> op) -> pplx::task<bool> 
            { 
                ...
                uvxx::streams::streambuf<CharType> buf = buffer;
...
public:
    ...
    {
        return uvxx::streams::_type_parser_base<CharType>::template _parse_input<std::basic_string<CharType>, std::string>(buffer, _accept_char, _extract_result);
    }
...
{
    typedef typename uvxx::streams::streambuf<CharType>::int_type int_type;
public:
...
private:
    static pplx::task<signed char> _get_char(streams::streambuf<CharType> buffer)
    {
        ...
        return buf.bumpc().then(
            [=](pplx::task<typename uvxx::streams::streambuf<CharType>::int_type> op) -> signed char
            {
                int_type val = op.get();
...
private:
    static pplx::task<unsigned char> _get_char(streams::streambuf<CharType> buffer)
    {
        uvxx::streams::streambuf<CharType> buf = buffer;
        return buf.bumpc().then(
            [=](pplx::task<typename uvxx::streams::streambuf<CharType>::int_type> op) -> unsigned char
            {
                int_type val = op.get();
                if (val == std::char_traits<CharType>::eof())
                    throw std::runtime_error("reached end-of-stream while constructing a value");
...
private:
    static pplx::task<char> _get_char(streams::streambuf<CharType> buffer)
    {
        uvxx::streams::streambuf<CharType> buf = buffer;
        return buf.bumpc().then(
            [=](pplx::task<typename uvxx::streams::streambuf<CharType>::int_type> op) -> char
            {
                int_type val = op.get();
                if (val == std::char_traits<CharType>::eof())
                    throw std::runtime_error("reached end-of-stream while constructing a value");
