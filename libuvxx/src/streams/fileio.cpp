...
using namespace uvxx::streams::details;
    ...
    namespace streams {
        namespace details {
            ...
            ****/
            ...
            {
                std::lock_guard<std::mutex> lck(_g_lock);
                if (!_g_scheduler)
                {
...
/// </remarks>
bool _close_fsb_nolock(_file_info **info, uvxx::streams::details::_filestream_callback *callback)
{
    if (callback == nullptr) return false;
...
bool _close_fsb(_file_info **info, uvxx::streams::details::_filestream_callback *callback)
{
    if (callback == nullptr) return false;
    if (info == nullptr || *info == nullptr) return false;
...
size_t _write_file_async(uvxx::streams::details::_file_info_impl *fInfo, uvxx::streams::details::_filestream_callback *callback, const uint8_t *ptr, size_t count, size_t position)
{
    ...
    pplx::create_task([=]() -> void
    {
        ...
        auto bytes_written = 0;//pwrite(fInfo->m_handle, ptr, count, abs_position);
        if (bytes_written == -1)
        {
           // callback->on_error(std::make_exception_ptr(utility::details::create_system_error(errno)));
        }
        ...
        {
           uvxx::pplx::extensibility::scoped_recursive_lock_t lock(fInfo->m_lock);
...
size_t _read_file_async(uvxx::streams::details::_file_info_impl *fInfo, uvxx::streams::details::_filestream_callback *callback, void *ptr, size_t count, size_t offset)
{
    auto sched = get_scheduler();
    sched->submit_io();

    uvxx::pplx::create_task([=]() -> void
    {
        auto bytes_read = 0;//pread(fInfo->m_handle, ptr, count, offset);
        if (bytes_read < 0)
        {
            //callback->on_error(std::make_exception_ptr(utility::details::create_system_error(errno)));
        }
...
/// <param name="count">The size (in bytes) of the buffer</param>
/// <returns>0 if the read request is still outstanding, -1 if the request failed, otherwise the size of the data read into the buffer</returns>
size_t _getn_fsb(uvxx::streams::details::_file_info *info, uvxx::streams::details::_filestream_callback *callback, void *ptr, size_t count, size_t charSize)
{
...
/// <param name="count">The size (in bytes) of the buffer</param>
/// <returns>0 if the read request is still outstanding, -1 if the request failed, otherwise the size of the data read into the buffer</returns>
size_t _putn_fsb(uvxx::streams::details::_file_info *info, uvxx::streams::details::_filestream_callback *callback, const void *ptr, size_t count, size_t charSize)
{
    if (callback == nullptr) return (size_t) -1;
    if (info == nullptr) return (size_t) -1;
...
/// <param name="ptr">A pointer to a buffer where the data should be placed</param>
/// <returns>0 if the read request is still outstanding, -1 if the request failed, otherwise the size of the data read into the buffer</returns>
size_t _putc_fsb(uvxx::streams::details::_file_info *info, uvxx::streams::details::_filestream_callback *callback, int ch, size_t charSize)
{
...
bool _sync_fsb(uvxx::streams::details::_file_info *info, uvxx::streams::details::_filestream_callback *callback)
{
    if (callback == nullptr) return false;
...
size_t _seekrdtoend_fsb(uvxx::streams::details::_file_info *info, int64_t offset, size_t char_size)
{
    if (info == nullptr) return (size_t) -1;
    ...
    auto newpos = 0;//lseek(fInfo->m_handle, static_cast<off_t>(offset * char_size), SEEK_END);
...
utility::size64_t _get_size(_In_ uvxx::streams::details::_file_info *info, size_t char_size)
{
    if (info == nullptr) return (size_t) -1;
    ...
    auto oldpos = 0;//lseek(fInfo->m_handle, 0, SEEK_CUR);

    if (oldpos == -1) return utility::size64_t(-1);

    auto newpos = 0;//lseek(fInfo->m_handle, 0, SEEK_END);
...
/// <param name="pos">The new position (offset from the start) in the file stream</param>
/// <returns>New file position or -1 if error</returns>
size_t _seekrdpos_fsb(uvxx::streams::details::_file_info *info, size_t pos, size_t)
{
    if (info == nullptr) return (size_t) -1;
...
/// <param name="pos">The new position (offset from the start) in the file stream</param>
/// <returns>New file position or -1 if error</returns>
size_t _seekwrpos_fsb(uvxx::streams::details::_file_info *info, size_t pos, size_t)
{
    if (info == nullptr) return (size_t) -1;
