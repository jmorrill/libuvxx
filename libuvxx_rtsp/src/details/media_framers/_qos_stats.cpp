...
qos_stats::qos_stats(): _percent_packet_loss(0),
...
void qos_stats::reset_statistics_interval(std::chrono::milliseconds reset_interval)
{
    _reset_statistics_interval = reset_interval;
}

std::chrono::milliseconds qos_stats::reset_statistics_interval()
{
    return _reset_statistics_interval;
}

double qos_stats::percent_packet_loss() const
{
    return _percent_packet_loss;
}

uint32_t qos_stats::expected_packet_count() const
{
    return _current_expected_packet_count;
}

uint32_t qos_stats::current_received_packet_count() const
{
    return _current_received_packet_count;
}

double qos_stats::total_kbytes_received() const
{
    return _total_kbytes_received;
}

void qos_stats::record_stats(RTPReceptionStats& stats)
{
